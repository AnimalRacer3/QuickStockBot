"""Unit tests for AlpacaClient — no real network calls (all requests mocked via respx)."""
...
def _cassette(name: str) -> dict | list:
...
class TestGetBars:
    ...
    def test_bar_fields_mapped_correctly(self) -> None:
        ...
        client = _make_client()
        bar = client.get_bars("AAPL", _START, _END)[0]
    ...
    def test_empty_bars_returns_empty_list(self) -> None:
        ...
        client = _make_client()
        assert client.get_bars("AAPL", _START, _END) == []
...
class TestSubmitOrder:
    ...
    def test_limit_order_includes_price(self) -> None:
        filled = _cassette("get_order_filled.json")
        limit_payload = {**_cassette("submit_order.json"), "type": "limit", "limit_price": "180.00"}
        respx.post("https://paper-api.alpaca.markets/v2/orders").mock(
            return_value=httpx.Response(200, json=limit_payload)
        )
        client = _make_client()
        order = client.submit_order(
            symbol="AAPL",
    ...
    def test_short_sell_side(self) -> None:
        short_payload = {**_cassette("submit_order.json"), "side": "sell", "qty": "5"}
        respx.post("https://paper-api.alpaca.markets/v2/orders").mock(
...
class TestStatusMapping:
    ...
    @respx.mock
    def test_all_status_values_map(self, raw_status: str, expected: OrderStatus) -> None:
        order_id = "61e69015-8549-4bfd-b9c3-01e75843f47d"
        payload = {**_cassette("get_order_filled.json"), "status": raw_status}
        respx.get(f"https://paper-api.alpaca.markets/v2/orders/{order_id}").mock(
            return_value=httpx.Response(200, json=payload)
        )
        client = _make_client()
        assert client.get_order(order_id).status == expected
...
class TestRetryBehavior:
    ...
    def test_raises_after_max_retries(self) -> None:
        ...
        with pytest.raises(httpx.HTTPStatusError):
            client.get_bars("AAPL", _START, _END)
