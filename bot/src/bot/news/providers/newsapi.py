...
class NewsAPIProvider(NewsProvider):
        ...
        for symbol in symbols:
            try:
                ...
                for item in resp.json().get("articles", []):
                    published_str: str = item.get("publishedAt", "")
                    try:
    ...
    @classmethod
    def from_config(cls, config: NewsConfig) -> Optional[NewsAPIProvider]:
        if not config.newsapi_key:
            logger.warning("Skipping NewsAPI: NEWSAPI_KEY not set")
            return None
