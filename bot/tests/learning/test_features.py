...
def test_extract_features_has_all_feature_order_keys():
    ...
    features = extract_features(
        state, sentiment_score=0.5, time_of_day_frac=0.25, sizing=500.0
    )
...
def test_extract_features_news_sentiment_matches_input():
    state = _make_state()
    features = extract_features(
        state, sentiment_score=0.85, time_of_day_frac=0.0, sizing=0.0
    )
...
def test_extract_features_zero_sentiment_clears_has_news():
    state = _make_state()
    features = extract_features(
        state, sentiment_score=0.0, time_of_day_frac=0.0, sizing=0.0
    )
...
def test_extract_features_macd_fields_match_state():
    macd = _make_macd(
        value=1.23, slope=-0.5, hist=0.07, favorability=0.3, eligible=True
    )
...
def test_extract_features_time_of_day_and_sizing():
    state = _make_state()
    features = extract_features(
        state, sentiment_score=0.0, time_of_day_frac=0.33, sizing=1234.5
    )
...
def test_features_to_vector_length():
    state = _make_state()
    features = extract_features(
        state, sentiment_score=0.5, time_of_day_frac=0.2, sizing=300.0
    )
...
def test_features_to_vector_order_matches_feature_order():
    ...
    features = extract_features(
        state, sentiment_score=-0.3, time_of_day_frac=0.9, sizing=999.0
    )
    vec = features_to_vector(features)
