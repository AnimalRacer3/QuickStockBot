...
_PAST_Z_HOUR = datetime(2024, 6, 10, 14, 31, 0, tzinfo=timezone.utc)  # 61 min after open
...
def _make_dummy_open_trade(symbol: str = "TSLA", shares: int = 10, entry: float = 100.0):
    ...
    order = Order(
        ...
        side=OrderSide.BUY, order_type=OrderType.MARKET, qty=Decimal(str(shares)),
        status=OrderStatus.FILLED, time_in_force=TimeInForce.DAY,
        submitted_at=datetime.now(timezone.utc), filled_avg_price=Decimal(str(entry)),
    )
    pos = OpenPosition(symbol, entry, shares, shares, entry)
    return TradeRecord(symbol=symbol, entry_order=order, entry_price=entry, shares=shares, position=pos)
...
class TestSizingIntegration:
    ...
    def test_greyed_default_enters_trade(self) -> None:
        ...
        cfg = self._cfg(daily_max_loss_pct=-2.0, stop_loss_pct=1.0, position_size_pct=50.0,
                        override_risk_per_trade=False)
        sess, client, _ = _session(cfg=cfg, equity=100_000, buying_power=100_000,
                                   fill_price=100.0, conviction=1.0)
        sess.start_day()
    ...
    def test_valid_lower_override_results_in_smaller_position(self) -> None:
        ...
        cfg_default = self._cfg(daily_max_loss_pct=-2.0, stop_loss_pct=1.0,
                                position_size_pct=100.0, override_risk_per_trade=False)
        sess_d, client_d, _ = _session(cfg=cfg_default, equity=100_000, buying_power=500_000,
                                       fill_price=100.0, conviction=1.0)
        sess_d.start_day()
        ...
        cfg_override = self._cfg(daily_max_loss_pct=-2.0, stop_loss_pct=1.0,
                                 position_size_pct=100.0, override_risk_per_trade=True,
                                 risk_per_trade_pct=0.5)
        sess_o, client_o, _ = _session(cfg=cfg_override, equity=100_000, buying_power=500_000,
                                       fill_price=100.0, conviction=1.0)
        sess_o.start_day()
    ...
    def test_override_equal_daily_falls_back_to_greyed(self) -> None:
        ...
        assert int(client_o.submitted_orders[0].qty) == int(client_d.submitted_orders[0].qty)

    def test_shares_less_than_1_skips(self) -> None:
        """Extremely small risk → max_risk too small for even 1 share → skip."""
        cfg = self._cfg(daily_max_loss_pct=-0.0001, stop_loss_pct=1.0, position_size_pct=100.0,
                        override_risk_per_trade=False)
        sess, client, _ = _session(cfg=cfg, equity=1_000, buying_power=10_000, conviction=1.0)
        sess.start_day()
        result = sess.run_cycle(["AAPL"], {"AAPL": self._bars()})
        assert result.trades_entered == 0
    ...
    def test_buying_power_cap_respected(self) -> None:
        ...
        cfg = self._cfg(daily_max_loss_pct=-2.0, stop_loss_pct=1.0, position_size_pct=100.0,
                        override_risk_per_trade=False)
        sess, client, _ = _session(cfg=cfg, equity=100_000, buying_power=500,
                                   fill_price=100.0, conviction=1.0)
        sess.start_day()
        bars = self._bars()
        result = sess.run_cycle(["AAPL"], {"AAPL": bars})
    ...
    def test_goalpost_trade_count_exposed(self) -> None:
        cfg = ExecutionConfig(daily_max_loss_pct=-3.0, override_risk_per_trade=True,
...
class TestProfitTargetCircuitBreaker:
    ...
    def test_single_trade_reaching_target_halts_day(self) -> None:
        ...
        result = sess.run_cycle(["AAPL", "TSLA"], {"AAPL": make_falling_bars(n=20),
                                                    "TSLA": make_falling_bars(n=20)})
        assert result.halted
        assert result.trades_entered == 0
...
class TestDumpExit:
    ...
    def test_dump_on_take_profit(self) -> None:
        sess, client, _ = _session(cfg=self._cfg_dump(), fill_price=100.0, conviction=1.0)
...
class TestTrailOffExit:
    ...
    def test_per_candle_fractional_scale_out(self) -> None:
        sess, client, _ = _session(cfg=self._cfg_trail("per_candle"), fill_price=100.0, conviction=1.0)
        if not self._enter(sess):
            return
    ...
    def test_dump_remainder_on_reversal_in_trail_off(self) -> None:
        ...
        pre = len(client.submitted_orders)
        sess.run_cycle(["AAPL"], {"AAPL": falling})
        post_sells = [o for o in client.submitted_orders[pre:] if o.side == OrderSide.SELL]
    ...
    def test_candle_pattern_trigger_sells_on_reconfirm(self) -> None:
        ...
        sess, client, _ = _session(cfg=self._cfg_trail("candle_pattern"), fill_price=100.0, conviction=1.0)
        if not self._enter(sess):
            return
...
class TestTrailingStop:
    ...
    def test_trailing_stop_not_triggered_above_stop(self) -> None:
        ...
        sess._open_trades["AAPL"] = _make_dummy_open_trade("AAPL", shares=10, entry=108.0)
...
class TestZHourCutoff:
    ...
    def test_z_cutoff_logged(self) -> None:
        cfg = ExecutionConfig(z_hour_cutoff=1.0, conviction_threshold=0.0,
                              exit_mode="dump", force_close_at_close=False, stop_loss_pct=1.0)
        now = _SESSION_OPEN + timedelta(minutes=20)
        sess, _, _ = _session(cfg=cfg, now=now, conviction=1.0)
        sess.start_day()
        bars = make_breakout_bars()
        result = sess.run_cycle(["AAPL"], {"AAPL": bars})
        assert any("z-hour" in msg.lower() or "cutoff" in msg.lower() for msg in result.log_messages)
...
class TestMarginDeferral:
    ...
    def test_min_equity_notice_logged(self) -> None:
        cfg = ExecutionConfig(min_account_equity_notice=2000.0, z_hour_cutoff=0.0,
                              exit_mode="dump", force_close_at_close=False,
                              conviction_threshold=0.0, stop_loss_pct=1.0)
        sess, _, _ = _session(cfg=cfg, equity=1500.0, conviction=1.0)
        sess.start_day()
        assert any("equity" in m.lower() or "notice" in m.lower() for m in sess._result.log_messages)
