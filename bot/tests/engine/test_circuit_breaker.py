...
class TestMaxLoss:
    def test_below_threshold_flatten_and_halt(self) -> None:
        ...
        action = check_daily_limits(
            state, realized_pnl=-2100.0, unrealized_pnl=0.0, config=cfg
        )
    ...
    def test_exactly_at_threshold_halts(self) -> None:
        ...
        state = _state(100_000)
        action = check_daily_limits(
            state, realized_pnl=-2000.0, unrealized_pnl=0.0, config=cfg
        )
        assert action == DailyAction.FLATTEN_AND_HALT
    ...
    def test_above_threshold_continues(self) -> None:
        ...
        state = _state(100_000)
        action = check_daily_limits(
            state, realized_pnl=-1000.0, unrealized_pnl=0.0, config=cfg
        )
    ...
    def test_combined_pnl_triggers(self) -> None:
        ...
        state = _state(100_000)
        action = check_daily_limits(
            state, realized_pnl=-1000.0, unrealized_pnl=-1200.0, config=cfg
        )
        assert action == DailyAction.FLATTEN_AND_HALT
    ...
    def test_already_halted_returns_halt(self) -> None:
        ...
        action = check_daily_limits(
            state, realized_pnl=0.0, unrealized_pnl=0.0, config=cfg
        )
        assert action == DailyAction.HALT
...
class TestProfitTarget:
    ...
    def test_reaches_target_flatten_and_halt(self) -> None:
        cfg = _cfg(daily_target_mode="stop", daily_profit_target_pct=3.0, flatten_on_profit_target=True)
        state = _state(100_000)
        action = check_daily_limits(
    ...
    def test_reaches_target_halt_only(self) -> None:
        cfg = _cfg(daily_target_mode="stop", daily_profit_target_pct=3.0, flatten_on_profit_target=False)
        state = _state(100_000)
        action = check_daily_limits(
            state, realized_pnl=3100.0, unrealized_pnl=0.0, config=cfg
        )
    ...
    def test_single_trade_can_hit_target(self) -> None:
        ...
        state = _state(100_000)
        action = check_daily_limits(
            state, realized_pnl=3500.0, unrealized_pnl=0.0, config=cfg
        )
        assert action == DailyAction.HALT
        assert state.halted
