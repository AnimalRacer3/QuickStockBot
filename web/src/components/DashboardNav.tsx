...
const LINKS = [
  { href: "/connect", label: "Connect", requiresConn: false },
  { href: "/live", label: "Live Actions", requiresConn: true },
  { href: "/tickers", label: "Active Tickers", requiresConn: true },
  { href: "/settings", label: "Settings", requiresConn: true },
  { href: "/lists", label: "Ticker Manager", requiresConn: true },
  { href: "/history", label: "Trade History", requiresConn: true },
  { href: "/calendar", label: "Calendar", requiresConn: true },
];

const BOTTOM_NAV = [
  { href: "/live", label: "Live", icon: Activity, center: false, requiresConn: true },
  { href: "/connect", label: "Connect", icon: Wifi, center: true, requiresConn: false },
  { href: "/settings", label: "Settings", icon: Settings, center: false, requiresConn: true },
  { href: "/lists", label: "Tickers", icon: LayoutList, center: false, requiresConn: true },
];
    ...
    <>
      ...
      <nav className="hidden sm:flex items-center gap-6 px-6 py-3 bg-gray-900 border-b border-gray-800 flex-wrap">
        ...
        <div className="flex gap-1 flex-wrap flex-1">
          {LINKS.map(({ href, label, requiresConn }) => {
            const locked = requiresConn && connectionState !== "connected";
            return locked ? (
              <span
                key={href}
                title="Connect to bot first"
                className="px-3 py-1.5 rounded text-sm text-gray-600 cursor-not-allowed select-none"
              >
                {label}
              </span>
            ) : (
              ...
              {LINKS.map(({ href, label, requiresConn }) => {
                ...
                return locked ? (
                  <span
                    key={href}
                    title="Connect to bot first"
                    className="px-3 py-2.5 rounded text-sm text-gray-600 cursor-not-allowed select-none"
                  >
                    {label}
                  </span>
                ) : (
      ...
      <nav className="fixed bottom-0 inset-x-0 z-40 flex sm:hidden items-end bg-gray-900 border-t border-gray-800 h-16">
        {BOTTOM_NAV.map(({ href, label, icon: Icon, center, requiresConn }) => {
          ...
          if (locked) {
            return (
              <span
                key={href}
                title="Connect to bot first"
                className="flex-1 flex flex-col items-center justify-center gap-1 pb-1 cursor-not-allowed"
              >
                <Icon size={20} className="text-gray-700" />
                <span className="text-[10px] leading-none text-gray-700">{label}</span>
              </span>
            );
          }
          return (
